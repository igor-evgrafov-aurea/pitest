...
public class DefaultCoverageGenerator implements CoverageGenerator {
    ...
    public DefaultCoverageGenerator(final File workingDir, final CoverageOptions coverageOptions,
            final LaunchOptions launchOptions, final CodeSource code, final CoverageExporter exporter,
    ...
    private static void verifyBuildSuitableForMutationTesting(final CoverageData coverage) {
        if (!coverage.allTestsGreen()) {
            LOG.warning("Error in supplied test cases: " + Help.FAILING_TESTS);
            // throw new PitHelpError(Help.FAILING_TESTS);
        }
    }

    private void gatherCoverageData(final Collection<ClassInfo> tests, final CoverageData coverage)
        ...
        final CoverageProcess process = new CoverageProcess(ProcessArgs.withClassPath(this.code.getClassPath())
                .andBaseDir(this.workingDir).andLaunchOptions(this.launchOptions).andStderr(logInfo())
                .andStdout(captureStandardOutIfVerbose()), this.coverageOptions, socket, filteredTests, handler);
        ...
        if (exitCode == ExitCode.JUNIT_ISSUE) {
            ...
            throw new PitError(
                    "Coverage generation minion exited abnormally. Please check that your classpath contains JUnit 4.6 or above.");
        } else if (!exitCode.isOk()) {
            LOG.severe("Coverage generator Minion exited abnormally due to " + exitCode);
            throw new PitError("Coverage generation minion exited abnormally due to " + exitCode);
        } else {
